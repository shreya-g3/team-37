...
def main():
    ...
    if transform == "arcsinh":
        # arcsinh(x/150) is fully invertible: x = 150*sinh(z). Unlike CLR,
        # no information was discarded in the forward transform, so this
        # recovers exact raw CODEX-scale protein values.
        pred_raw = 150.0 * np.sinh(pred_unstd)
        ...
        print("(values are inverse-transformed back to raw CODEX protein scale)")
    else:
