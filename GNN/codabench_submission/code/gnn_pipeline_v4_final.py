...
class MultiScaleSpatialGNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_local: torch.Tensor, edge_regional: torch.Tensor) -> torch.Tensor:
        ...
        # Gradient checkpointing (training only): recompute activations during
        # backward instead of storing all n_layers of them. Trades ~30-50% extra
        # forward compute (cheap on this GPU) for a big memory cut, so we can run
        # the full intended hidden/n_layers spec without OOM on a 16GB T4.
        hl = h0
...
def combined_loss(yp: torch.Tensor, yt: torch.Tensor, w: float = 0.8) -> torch.Tensor:
    """w x MSE + (1-w) x (1 - mean Pearson r).
    pearson_loss is scale/shift-invariant (it only rewards correlation
    *shape*, not matching absolute magnitude), so MSE is weighted heavily
    to also force the model to get the absolute scale right, not just the
    correlation. weight_decay is set a bit higher as insurance against
    overfitting now that MSE gets more say."""
