...
def xgboost_tuning(
    ...
    for j in sampled_protein_idx:
        base_model = XGBRegressor(
            tree_method="hist",
            device="cpu",          # for kaggle "cuda", to use GPU
...
def xgboost_tuning_svd(
        rna_path,  # "../preprocessing/outputs/rna_hvg.h5ad"
        pro_path,  # "../preprocessing/outputs/protein_data.h5ad"
        cv_split_path,  # "../preprocessing/outputs/cv_splits.json"
        n_components=50,  # SVD components -- match whatever xgboost_svd is being run with
        A=None,  # sparse (bins x bins) adjacency matrix from neighbourhood_means(), or None for no spatial features
    ...
    """
    global hyperparameter search for xgboost_svd.py
    runs a small RandomizedSearchCV on a few representative proteins, using only training data from fold 1
    features are truncated-SVD-reduced (and optionally spatially averaged, matching xgboost_svd)
    before tuning, fit on fold 1's training split only, so the search sees the
    same kind of feature matrix xgboost_svd will actually train on
    best parameters across sampled proteins are averaged - parameters for final model
    proteins are selected to include a variation of expression so avoid overfitting to easy/hard proteins
    """
    ...
    # fit truncated SVD on fold 1's train split only -- matches how xgboost_svd
    # fits SVD fresh per fold, so the tuning search sees a realistic feature matrix
    svd = TruncatedSVD(n_components=n_components, random_state=svd_random_state)
    X_tr_svd = svd.fit_transform(X_tr_raw)
    print(f"SVD cumulative explained variance on tuning split: {svd.explained_variance_ratio_.sum():.3f}")

    if A is not None:
        # subset adjacency to fold 1's train indices only, same as xgboost_svd
    ...
    print(f"\nRecommended global xgb_params (hard-code into xgboost_svd's xgb_params arg):")
    print(best_params)
