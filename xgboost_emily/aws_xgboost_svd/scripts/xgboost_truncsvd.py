...
def xgboost_svd(
    ...
    n_components,           # number of components for truncated SVD (50)
    A=None,                 # sparse (bins x bins) adjacency/weight matrix from neighbourhood_means(), or None for no spatial features
    svd_random_state=0,     # seed for SVD reproducibility
    ...
    uses truncated SVD for feature reduction
    spatial neighbourhood features are calculated after reducing
    ...
    if use_spatial and A.shape[0] != X.shape[0]:
        raise ValueError(
            f"adjacency matrix shape {A.shape} does not match "
            f"rna cell count {X.shape[0]} -- check both were built from "
            f"the same rna file / cell ordering"
        )
    ...
    default_params = dict(
        ...
        learning_rate=0.019,         # default:0.05, tuning:0.053, 0.019
        subsample=0.76,              # default:0.8, tuning:0.86, 0.76
        colsample_bytree=0.8,        # default:0.8, tuning: 0.68, 0.8
        tree_method="hist",
        device=device,
        random_state=42,
        n_jobs=-1)
    ...
    fold_explained_var = np.zeros(n_splits)  # cumulative SVD explained variance per fold
    ...
    for split in splits:
        ...
        # fit truncated SVD on train fold only, transform val fold (prevents leakage of val-set variance structure into train features)
        ...
        if use_spatial:
            # subset adjacency to within-fold neighbours only
            A_tr = A[np.ix_(train_idx, train_idx)]
            A_val = A[np.ix_(val_idx, val_idx)]

            # neighbourhood means calculation from adjacency matrix
            X_tr_neighbour = A_tr @ X_tr_svd
            X_val_neighbour = A_val @ X_val_svd

            # concatenate self (X_*_svd) and neighbourhood-mean
            X_tr = np.hstack([X_tr_svd, X_tr_neighbour])
            X_val = np.hstack([X_val_svd, X_val_neighbour])
        else:
            X_tr = X_tr_svd
            X_val = X_val_svd
    ...
    # save results
    results_df.to_csv(f"{out_path}/xgb_svd_results{suffix}.csv", index=False)
    per_protein_results_df.to_csv(f"{out_path}/xgb_svd_per_protein_metrics{suffix}.csv", index=False)
