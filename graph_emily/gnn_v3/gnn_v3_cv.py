"""
5-fold spatial CV for ResidualGraphSAGE model

conv_type="sage" (default)
conv_type="gat" for GATConv-based attention layers

Inputs:
    rna_path: preprocessed RNA
    protein_path: preprocessed protein data
    cv_split_path: cv splits json from cv_split_patches()
    out_dir: "results"

Outputs:
    fold{N}_gnn_v3.csv: per-fold, per-protein metrics
    gnn_v3_cv_results.csv: overall summary
    gnn_v3_cv_per_protein_metrics.csv: per-protein metrics averaged across folds
    gnn_v3_model_fold{N}.pt: saved model weights + config per fold
    svd_model_gnn_v3_fold{N}.pkl: saved SVD per fold
...
from gnn_v3 import ResidualGraphSAGE, build_knn_graph, combined_loss, WarmupCosineScheduler, to_dense
from cv_split_patches import load_cv_split

# current config
DEFAULT_V3_PARAMS = dict(
    hidden=256,
    n_layers=4,
    dropout=0.3,
    lr=3e-4,
    weight_decay=1e-3,
    warmup=10,
    loss_w=0.8,  # combined_loss's MSE weight; (1 - loss_w) goes to the Pearson term
    conv_type="sage",  # "sage" or "gat"
    use_jk=False,
    gat_heads=2,  # only used when conv_type="gat"
)
...
def run_gnn_v3_cv(
    ...
    """
    Cross-validated ResidualGraphSAGE
    per-fold truncated SVD
    """
    os.makedirs(out_dir, exist_ok=True)
    device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")

    run_params = dict(DEFAULT_V3_PARAMS)
    if params:
    ...
    for split in splits:
        ...
        # per-fold SVD, fit on train bins only, transform all
        ...
        with open(f"{out_dir}/svd_model_gnn_v3_fold{fold}.pkl", "wb") as f:
            pickle.dump(svd, f)
        ...
        save_model(model, f"{out_dir}/gnn_v3_model_fold{fold}.pt")
        ...
        fold_results.to_csv(f"{out_dir}/fold{fold}_gnn_v3.csv", index=False)
    ...
    results_df.to_csv(f"{out_dir}/gnn_v3_cv_results.csv", index=False)
    per_protein_results_df.to_csv(f"{out_dir}/gnn_v3_cv_per_protein_metrics.csv", index=False)
