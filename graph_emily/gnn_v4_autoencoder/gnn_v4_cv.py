"""
5-fold spatial CV gnn v4
ResidualGraphSAGE + autoencoder pipeline
hyperparameter config. from bayesian optimisation search
pearsonr/r2/rmse + per-protein metrics

input best_config.json from hyperparameter search

imports ResidualGraphSAGE, build_knn_graph, combined_loss, and WarmupCosineScheduler from gnn v4
imports rna_autoencoder

Inputs:
    rna_path: preprocessed RNA (no SVD)
    protein_path: preprocessed protein data
    cv_split_path: cv splits json from cv_split_patches()
    out_dir:"results"

Outputs:
    fold{N}_gnn_v4.csv: per-fold, per-protein metrics
    gnn_v4_cv_results.csv: overall summary (one row)
    gnn_v4_cv_per_protein_metrics.csv: per-protein metrics averaged across folds
    gnn_v4_model_fold{N}.pt: saved model weights + config per fold
    autoencoder_model_fold{N}.pt: saved autoencoder per fold
...
from rna_autoencoder import fit_autoencoder, encode, save_autoencoder
from cv_split_patches import load_cv_split

# config from gnn v3 run
DEFAULT_V4_PARAMS = dict(
    ...
    conv_type="sage",
    use_jk=False,
    gat_heads=2,        # for conv type "gat"
)

K_NEIGHBORS = 8
LATENT_DIM = 128
AE_HIDDEN_DIMS = (1024, 512)
AE_EPOCHS = 100
MAX_EPOCHS = 300
PATIENCE = 20
SEED = 0
...
def train_with_early_stopping(X, Y, edge_index, train_mask, holdout_mask, params,
    ...
    """
    training loop for one loop
    AdamW + warmup/cosine schedule + combined loss, early-stopping on fold holdout
    ...
    for epoch in range(max_epochs):
        ...
        loss = combined_loss(out[train_mask_dev], y_t[train_mask_dev], w=loss_w)
        loss.backward()
        optimizer.step()
        sched.step()
        ...
        with torch.no_grad():
            out = model(x_t, edge_index_dev)
            val_loss = combined_loss(out[holdout_mask_dev], y_t[holdout_mask_dev], w=loss_w).item()
...
def run_gnn_v4_cv(
    ...
    """
    cross-validated ResidualGraphSAGE + autoencoder embedding
    add hyperparameter configurations to overwrite default_v4_params (best_config.json or manually)
    """
    ...
    for split in splits:
        ...
        # per-fold autoencoder, fit on train bins only, transform all
        ae = fit_autoencoder(X_raw[train_idx], latent_dim=latent_dim, hidden_dims=ae_hidden_dims,
                              epochs=ae_epochs, device=device, verbose=True)
        X_feat = encode(ae, X_raw, device=device)
        save_autoencoder(ae, f"{out_dir}/autoencoder_model_fold{fold}.pt")
        ...
        model, n_epochs, Y_pred_all = train_with_early_stopping(
            X_feat, Y, edge_index, train_mask, holdout_mask, run_params, device,
        ...
        del ae, X_feat, model, Y_pred_all, Y_val_pred, Y_val
        gc.collect()
    ...
    results_df.to_csv(f"{out_dir}/gnn_v4_cv_results.csv", index=False)
    per_protein_results_df.to_csv(f"{out_dir}/gnn_v4_cv_per_protein_metrics.csv", index=False)
