...
added
    ...
    - combined_loss (0.8*MSE + 0.2*(1 - mean Pearson r))
...
class ResidualGraphSAGE(nn.Module):
    """Single-branch stack of ResidualSAGEBlocks over ONE kNN graph."""
    def __init__(self, in_dim, out_dim, hidden=256, n_layers=4, dropout=0.3):
        ...
        self.blocks = nn.ModuleList([ResidualSAGEBlock(hidden, dropout) for _ in range(n_layers)])
        ...
        self._config = dict(in_dim=in_dim, out_dim=out_dim, hidden=hidden,
...
def find_n_epochs(X, Y, edge_index, cv_splits, fold, model_cfg, opt_cfg, device,
    ...
    for epoch in range(max_epochs):
        ...
        loss = combined_loss(out[train_mask], y_t[train_mask])
        loss.backward()
        optimizer.step()
        sched.step()
...
def run_gnn_v4(rna_train_path, pro_train_path, rna_val_path, pro_stats_path, cv_split_path,
               ...
               lr=3e-4, weight_decay=1e-3, warmup=10, max_epochs=1000, patience=30,
