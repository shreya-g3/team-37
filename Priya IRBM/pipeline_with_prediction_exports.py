...
def select_hvg_train_only(
    ...
    """
    Select HVGs using only training fold.

    Robust for:
    - tiny smoke tests (sample_n=100)
    - sparse Visium HD matrices
    - genes with many zero counts
    - Scanpy binning failures
    """
    ...
    train_rna = train_rna[:, keep].copy()

    if train_rna.n_vars == 0:
        raise RuntimeError("All genes removed during HVG filtering.")

    # Small training set -> variance ranking
    # Small training set -> variance ranking
    if train_rna.n_obs < 200:
        X = dense_float32(train_rna.X)
    ...
    # -------------------------------------------------------
    # Try Scanpy Seurat v3
    # -------------------------------------------------------
    try:
        sc.pp.highly_variable_genes(
            train_rna,
            flavor="seurat_v3",
            n_top_genes=min(n_hvg, train_rna.n_vars),
            subset=False,
        )

        hvgs = train_rna.var_names[
            train_rna.var["highly_variable"]
        ].tolist()

    except Exception as e:
        print(
            f"    seurat_v3 HVG failed: {type(e).__name__}: {e}"
        )


    # -------------------------------------------------------
    # Try Scanpy Cell Ranger
    # -------------------------------------------------------
    if len(hvgs) == 0:
        ...
        try:
            ...
            print(
                f"    cell_ranger HVG failed: {type(e).__name__}: {e}"
            )


    # -------------------------------------------------------
    # Guaranteed fallback:
    # variance ranking (no Scanpy binning)
    # -------------------------------------------------------
    if len(hvgs) == 0:

        print(
            "    Using variance based HVG fallback."
        )
...
def make_fold_features(
    ...
    if feature_selection == "variance":
        ...
        train_rna = rna_norm[train_idx].copy()
        sc.pp.filter_genes(train_rna, min_cells=3)
        x_for_scores = dense_float32(train_rna.X)
        y_for_scores = y[train_idx]
        if x_for_scores.shape[1] == 0:
            raise RuntimeError("No genes remain after filtering.")
        x_centered = x_for_scores - x_for_scores.mean(axis=0, keepdims=True)
        y_centered = y_for_scores - y_for_scores.mean(axis=0, keepdims=True)
        ...
        top = np.argsort(scores)[::-1][: min(n_hvg, len(scores))]
        hvgs = train_rna.var_names[top].tolist()
    else:
        raise ValueError(
...
def export_fold_protein_predictions(
    ...
    pred_expression = np.sinh(pred) * target_scale
    true_expression = np.sinh(true) * target_scale
    ...
    for i, protein in enumerate(protein_names):
        ...
        df = pd.DataFrame({
            ...
            "True": true_expression[:, i],

            "Predicted": pred_expression[:, i],

        })

        outfile = predictions_dir / f"{safe_filename(protein)}_predicted.csv"
...
def run_pipeline(config: PipelineConfig) -> pd.DataFrame:
    ...
    predictions_dir = out_dir / "predicted_proteins"
    ...
    for fold_no, (train_idx, valid_idx) in enumerate(splits, start=1):
        ...
        export_fold_protein_predictions(
        predictions_dir=predictions_dir,
        obs_names=rna_norm.obs_names[valid_idx],
        pred=pred,
        true=y_valid,
