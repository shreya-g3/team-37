...
class SpatialBranch(nn.Module):
    ...
    pre-projection: down to GAT_PROJ_DIM first

    Distance weights are passed as edge_attr (shape [E, 1])

    edge_dropout randomly drops a fraction of edges per forward pass during
    training
    """
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # re-sample which edges get dropped for the second layer, don't reuse
        # layer 1's mask - otherwise same "missing" edges
        # compound across both layers instead of each layer seeing an
        # independent view of the neighbourhood
...
class ExpressionBranch(nn.Module):
    ...
    Own features are concatenated with a distance/similarity-weighted
    neighbour average
    """
    ...
    def _weighted_agg(x, edge_index, edge_weight):
        ...
        PyG convention: edge_index[0] = source (neighbour whose features get
        gathered), edge_index[1] = target (node whose bucket accumulates the
        weighted average).
        """
        neighbor, target = edge_index
        weighted = x[neighbor] * edge_weight.unsqueeze(1)
        out = torch.zeros_like(x)
        out.scatter_add_(0, target.unsqueeze(1).expand_as(weighted), weighted)
        wsum = torch.zeros(x.shape[0], 1, device=x.device)
        wsum.scatter_add_(0, target.unsqueeze(1), edge_weight.unsqueeze(1))
        return out / wsum.clamp(min=1e-8)


class FusionBlock(nn.Module):
    """
    Bilinear-gated fusion of the two branches

    Three signals feed the gate:
      1. the raw h_spatial, h_expr (unprojected)
      2. their projected elementwise product (bilinear interaction term)
      3. a residual pass-through path (raw concat projected)

    hs, he (the projected branches) feed the interaction term and
    the final blend

    fused = gate * hs + (1 - gate) * he + residual_proj(concat)
    then LayerNorm + ELU + Dropout
    """
...
class DualBranchGNN(nn.Module):
    ...
    def __init__(self, in_dim, out_dim,
        ...
        self.log_var = nn.Parameter(torch.zeros(out_dim)) if use_uncertainty_weighting else None
    ...
    def forward(self, x_spatial, spatial_ei, spatial_ew, x_expr, expr_ei, expr_ew,
                n_seed=None, return_gate=False):
        """
        x_spatial / x_expr are separate.
        sb.x and eb.x are not the same local node ordering beyond the seed prefix

        n_seed: when set, both branch outputs are sliced to their first
        n_seed rows before fusion. Both loaders share an identical seed-node order (make_loaders in
        train.py) - rows 0..n_seed-1 are the same global
        nodes in the same order in both x_spatial and x_expr.
        Leave as None for a full-batch call
        """
        h_spatial = self.spatial_branch(x_spatial, spatial_ei, spatial_ew)
        h_expr = self.expr_branch(x_expr, expr_ei, expr_ew)
        if n_seed is not None:
            h_spatial = h_spatial[:n_seed]
            h_expr = h_expr[:n_seed]
...
def pearson_loss(yp, yt, valid=None, eps=1e-8):
    ...
    mean/covariance/variance are computed over valid rows per protein,
    """
    if valid is None:
...
def combined_loss(yp, yt, w=0.8, missing_mask=None):
    ...
    value) excludes those entries from both terms.
    """
