...
# RNA preprocessing: normalise + log1p + SVD

def preprocess_and_reduce_rna(rna_train, rna_val,
...
class DualGraphModel(nn.Module):
    def __init__(self, in_dim, out_dim,
        ...
        gat_latent = gat_heads * gat_out_per_head
    ...
    def forward(self, x,
                ...
                expr_ei,    expr_ew):
        z = self.vgae.encode(x, spatial_ei, edge_attr=spatial_ew)
...
def run_gnn_v5(
    ...
    sage_hidden=256, n_sage_layers=2, dropout=0.3,
    gat_proj_dim=GAT_PROJ_DIM,
    gat_heads=GAT_HEADS,
    gat_out_per_head=GAT_OUT_PER_HEAD,
    ...
    print("\n--- SVD ---")
    X_train_svd, X_val_svd, svd = preprocess_and_reduce_rna(
        rna_train, rna_val, n_components=n_components
    )
